...
(async function() {
  ...
  async function saveVehicleCrewProblems(problemsById) {
    const plain = {};
    for (const [id, {message: message}] of problemsById) plain[id] = message;
  ...
  async function renameVehicle(vehicleId, newName) {
    const res = await fetch(`/vehicles/${vehicleId}/editName`, {
      credentials: "same-origin"
    });
  ...
  async function renameBuilding(buildingId, newName) {
    const res = await fetch(`/buildings/${buildingId}/edit`, {
      credentials: "same-origin"
    });
  ...
  async function initModal() {
    ...
    pageJQuery(modal).on("show.bs.modal", () => {
      ...
      if (!howItWorksSeen) {
        renderHowItWorksScreen(renderMainMenu, {
          firstOpen: true
        });
        return;
      }
  ...
  async function renderChangelogScreen(goBack) {
    ...
    document.getElementById("vn-btn-back").addEventListener("click", goBack);
    try {
      const res = await fetch(`${CHANGELOG_URL}?_=${Date.now()}`, {
        cache: "no-store"
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  function renderNameForm(selectedStations) {
    ...
    document.getElementById("vn-btn-reset-template").addEventListener("click", async () => {
      const confirmed = confirm("Achtung: Das setzt die Namens-Bausteine-Vorlage (Text 1, Fahrzeugtyp-Name, Text 2, " + "Nummer) wieder auf die Standardeinstellung zurück. Fortfahren?");
      if (!confirmed) return;
  ...
  async function buildStorage(buildingId, storageId, currency) {
    const csrfToken = getCsrfTokenOrThrow(buildingId);
    const res = await fetch(`/buildings/${buildingId}/storage_upgrade/${currency}/${storageId}?redirect_building_id=${buildingId}`, {
      method: "POST",
      credentials: "same-origin",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
  ...
  function renderVehicleSellConfirmScreen({vehicleId: vehicleId, vehicleName: vehicleName, stationName: stationName, goBack: goBack}) {
    setModalWidth(MODAL_WIDTH_COMPACT);
    const body = document.getElementById("vehicle-naming-modal-body");
  ...
  async function fetchPersonalPage(buildingId) {
    const res = await fetch(`/buildings/${buildingId}/personals`, {
      credentials: "same-origin"
    });
  ...
  async function fetchSchoolPageInfo(schoolId, occupied, slug = null) {
    const res = await fetch(`/buildings/${schoolId}`, {
      credentials: "same-origin"
    });
  ...
  async function fetchAvailablePersonnelForEducation(stationId, slug) {
    const res = await fetch(`/buildings/${stationId}/schooling_personal_select`, {
      credentials: "same-origin"
    });
  ...
  async function fetchVehicleFmsReal(vehicleId) {
    const res = await fetch(`/api/v2/vehicles/${vehicleId}`, {
      credentials: "same-origin"
    });
  ...
  async function fetchVehicleAssignmentPage(vehicleId) {
    const res = await fetch(`/vehicles/${vehicleId}/zuweisung`, {
      credentials: "same-origin"
    });
    if (!res.ok) {
  ...
  async function setVehicleFms(vehicleId, fmsStatus) {
    const res = await fetch(`/vehicles/${vehicleId}/set_fms/${fmsStatus}`, {
      credentials: "same-origin"
    });
  ...
  async function renderVehicleCrewScreen(goBack = renderMainMenu) {
    ...
    for (const [id, message] of Object.entries(persistedProblems)) {
      ...
      if (vehicle) problemsById.set(id, {
        vehicle: vehicle,
        message: message
      });
    }
  ...
  async function renderStationBlueprintEditScreen(blueprintId, goBack) {
    ...
    function updatePersonnelRequirements() {
      ...
      const totals = computeBlueprintPersonnelRequirements({
        vehicles: vehicles
      });
      const totalSum = [ ...totals.values() ].reduce((a, b) => a + b, 0);
      const rows = [ ...totals.entries() ].sort((a, b) => (qualifications[a[0]] || a[0]).localeCompare(qualifications[b[0]] || b[0], "de")).map(([slug, count]) => `<tr><td>${count}</td><td>${escapeHtml(qualifications[slug] || slug)}</td></tr>`).join("");
      document.getElementById("vn-bp-personnel-body").innerHTML = rows || `<tr><td colspan="2" class="text-muted">Keine Ausbildung erforderlich.</td></tr>`;
  ...
  async function main() {
    ...
    const results = await Promise.allSettled([ initModal(), initVehicleTypeCaptions(), initNamesStore(), initHowItWorksFlag() ]);
    results.forEach((r, i) => {
