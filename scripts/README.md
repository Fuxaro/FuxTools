...
Schaltet in einer bereits kommentarfreien Datei nur noch das kosmetische `CHANNEL`-Label auf
`"stable"` um (zeigt sich nur in Titel/Fußzeile/Einstellungen als "Beta"/"Stable"-Badge).
`@updateURL`/`@downloadURL` zeigen in beiden Repos bereits auf `main` – es gibt keinen
öffentlichen `beta`-Branch als eigenen Auto-Update-Kanal mehr (Beta-Tests laufen über eine
manuell installierte Kopie aus dem privaten Beta-Repo, siehe Haupt-README).
